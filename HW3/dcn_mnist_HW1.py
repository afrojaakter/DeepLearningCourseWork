...
def main():
    ...
    cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y_conv), 
                                                     reduction_indices = [1]))
    train_step = tf.train.AdamOptimizer(1e-4, 0.99).minimize(cross_entropy)
    ...
    for i in range(max_step):
        batch = mnist.train.next_batch(128) # make the data batch, which is used in the training iteration.
